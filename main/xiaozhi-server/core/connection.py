...
def with_state_system_note(conn, dialogue: list):
    """
    在对话最前面塞一条系统状态注记：[state] idle|training|rest
    避免重复注入（如果首条已是 [state] 就不再添加）
    """
    if dialogue and isinstance(dialogue[0], dict):
        first = dialogue[0]
        if first.get("role") == "system" and isinstance(first.get("content",""), str) \
           and first["content"].startswith("[state] "):
            return dialogue

    mode = get_state(conn, "idle")  # idle | training | rest
    return [{"role": "system", "content": f"[state] {mode}"}] + dialogue
...
class ConnectionHandler:
    ...
    def chat(self, query, tool_call=False, depth=0):
        ...
        try:
            ...
            llm_input = with_state_system_note(self, llm_input)  # ✅ 在这里统一加状态
            # ✅ 打一条调试日志，确认当前传给 LLM 的状态
            try:
                if llm_input and isinstance(llm_input[-1], dict) and llm_input[-1].get("role") == "user":
                    mode = get_state(self, "idle")
                    llm_input[-1]["content"] = f"[state] {mode}\n{llm_input[-1]['content']}"
                    self.logger.bind(tag=TAG).debug("[LLM][STATE_NOTE] state prefixed into user message")
            except Exception:
                pass
            ...
            if self.intent_type == "function_call" and functions is not None:
                ...
                llm_responses = self.llm.response(self.session_id, llm_input)
